...
then 
    ...
    cp conf/{dqn,experiment}.json ${RESULT_DIR}/conf
    ...
    # time python3 src/common/postgres_controller.py --config=conf/dqn.json --experiment_config=conf/experiment.json --result_dir=${RESULT_DIR} --generate_workload=False --with_agent=False --default_baseline=False --seed=${SEED};
    #
    # run dqn
    #
    echo "#### RUNNING DQN ON WORKLOAD ####" ;
    time python3 src/common/postgres_controller.py --config=conf/dqn.json --experiment_config=conf/experiment.json --result_dir=${RESULT_DIR} --generate_workload=False --seed=${SEED} &> ${RESULT_DIR}/dqn.log ;
    mv ${RESULT_DIR}/dqn.log ${RESULT_DIR}/dqn ;
    #
    # run spg
    #
    # echo "#### RUNNING SPG ON WORKLOAD ####" ;
    # time python3 src/common/postgres_controller.py --dqn=False --config=conf/spg.json --experiment_config=conf/experiment.json --result_dir=${RESULT_DIR} --generate_workload=False --seed=${SEED} &> ${RESULT_DIR}/spg.log ;
    # mv ${RESULT_DIR}/spg.log ${RESULT_DIR}/spg ;
    #
    # run tuner
    #
    echo "#### RUNNING TUNER ON WORKLOAD ####" ;
    time python src/baseline/postgres_tuner.py --experiment_config=conf/experiment.json --data_dir=${RESULT_DIR} &> ${RESULT_DIR}/tuner.log ;
...
then 
    ...
    #
    echo "#### RUNNING DQN ON WORKLOAD ####" ;
    time python3 src/common/postgres_controller.py --dqn=True --config=conf/dqn.json --experiment_config=conf/experiment.json --result_dir=${RESULT_DIR} --generate_workload=False --seed=${SEED} &> ${RESULT_DIR}/dqn.log ;
    mv ${RESULT_DIR}/dqn.log ${RESULT_DIR}/dqn ;
    #
    # run spg
    #
